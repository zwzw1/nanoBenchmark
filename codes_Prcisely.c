...
void inline undertestFunc(void)
{
    rdtsc();
}
...
uint64_t var_calc(uint64_t *inputs, int size)
{
    ...
    return (temp_var);
    overflow:
    printk(KERN_ERR "\n\n>>>>>>>>>>>>>> CRITICAL OVERFLOW ERRORIN var_calc!!!!!!\n\n");
    return -EINVAL;
}
static int __init hello_start(void)
{
    ...
    for (j=0; j<BOUND_OF_LOOP; j++) {
        ...
        max_deviation: %llu ;max time: %llu ;min time: %llu", j, variances[j], max_dev,max_time, min_time);
